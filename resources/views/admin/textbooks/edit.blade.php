        ...
        <form action="{{ route('textbooks.update', $textbook->id) }}" method="post">
            ...
            <div class="row_input_group_3">
                <div class="input_group">
                    ...
                    <select name="student_id" id="student_id" class="searchable_select">
                        ...
                        @foreach($students as $student)
                            <option value="{{ $student->id }}" {{ old('student_id', $textbook->student_id) == $student->id ? 'selected' : '' }}>{{ $student->registration_number.' - '.$student->first_name.' '.$student->last_name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="input_group">
                    ...
                    <input type="text" name="book_number" id="book_number" value="{{ old('book_number', $textbook->book_number) }}">
                    <span class="inline_alert">{{ $errors->first('book_number') }}</span>
                </div>

                <div class="input_group">
                    <label for="book_name">Book Name</label>
                    <input type="text" name="book_name" id="book_name" value="{{ old('book_name', $textbook->book_name) }}">
                ...
                <div class="input_group">
                    ...
                    <select name="status" id="status">
                        ...
                        @foreach(App\Models\Textbooks::STATUSES as $status)
                            <option value="{{ $status }}" {{ old('status', $textbook->status) == $status ? 'selected' : '' }}>{{ ucfirst($status) }}</option>
                        @endforeach
                    </select>
            ...
            <div class="buttons">
                ...
                <button type="button" class="delete_btn" onclick="deleteItem({{ $textbook->id }}, 'textbook');"
